def add_to_dict(a_dict, word="", definition=""):
              # a_dict(list), word(string), definition(string)
  if type(a_dict) is not dict:  #만약 a_dict가 dictionary 타입이 아니라면 
    print("You need to send a dictionary. You sent:", type(a_dict))
  elif word == '' or definition == '': # a_dict가 dictionary 타입이고(elif) word랑 definition 중 하나라도 None라면
    print("You need to send a word and a definition.")
  else: #a_dict가 dictionary 타입이고, word나 definition이 None가 아닌데,
    if word in a_dict: #word가 a_dict에 포함되어 있다면
      print(f"{word} is already on the dictionary. Won't add.")
    else: #word가 a_dict에 포함되어 있지 않다면
      a_dict[word] = definition # word를 definition에 추가시키고
